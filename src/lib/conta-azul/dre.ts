...
export const DRE_STRUCTURE: DreLine[] = [
  ...
  { id: "OS", label: "(-) Outras Saídas", kind: "sum", sign: -1, prefixes: ["OS", "SO"] },
  ...
  { id: "IN", label: "(-) Investimentos", kind: "sum", sign: -1, prefixes: ["IV", "EM", "IN"] },
...
function passaVisao(row: ContaRow, visao: Visao, ano: number, mes: number): boolean {
  if (visao === "realizado") {
    // Já pago — período de referência é o vencimento do título (competência do mês).
