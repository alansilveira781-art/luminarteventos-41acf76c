...
function FinanceiroDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const finalizadas = demandas.filter((c: any) => c.status === "finalizado").length;
    const emAndamento = demandas.filter((c: any) => !["finalizado", "negada"].includes(c.status)).length;
  ...
  const porStatus = useMemo(() => {
    ...
    const map = new Map<string, number>();
    demandas.forEach((c: any) => map.set(c.status, (map.get(c.status) ?? 0) + 1));
    return Array.from(map.entries()).map(([k, v]) => ({ nome: (labels as any)[k] ?? k, valor: v }));
  }, [demandas]);
  ...
  return (
    <>
      <PageHeader title="Dashboard de AquisiÃ§Ãµes" description="Indicadores e gráficos do período selecionado" />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ChartCard title="AquisiÃ§Ãµes por tipo (R$)">
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
      ...
      <Card className="p-4 mt-4">
        <div className="flex items-baseline justify-between mb-3">
          <div className="text-sm font-semibold">Demonstrativo de AquisiÃ§Ãµes (DRE)</div>
