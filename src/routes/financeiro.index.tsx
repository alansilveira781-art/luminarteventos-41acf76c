...
function DemandasKanban() {
  ...
  async function advanceToStatus(
    ...
    if (status === "a_receber" && !tiposDespesa.vaiParaRecebimento(demanda.tipo_demanda)) {
      toast.error('Este tipo de aquisição não gera recebimento em Estoque ou Patrimônio, então não pode ir para "A Receber".');
      return;
    }
    const id = demanda.id;
...
function Card({
  ...
  // Aquisição parcelada (datas de pagamento diferentes) e ainda em aberto → âmbar
