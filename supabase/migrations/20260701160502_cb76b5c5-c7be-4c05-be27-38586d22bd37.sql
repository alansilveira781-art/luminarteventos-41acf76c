...
BEGIN
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    IF v_resp_origem IS NOT NULL AND auth.uid() IS DISTINCT FROM v_resp_origem THEN
      RAISE EXCEPTION 'Apenas o responsável definido para o status "%" pode retirar o card desse status.', OLD.status
        USING ERRCODE = 'insufficient_privilege';
    END IF;
  END IF;
