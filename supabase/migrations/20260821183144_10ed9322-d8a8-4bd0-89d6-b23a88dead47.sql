...
BEGIN
  ...
  SELECT EXISTS (
    ...
    FROM public.profiles p
    WHERE p.id = auth.uid()
      AND lower(coalesce(p.email, '')) = 'natanael@luminarteventos.com.br'
  ) INTO v_is_natanael;
  ...
  -- Atalho do Natanael: Em Andamento -> Finalizado (pula "A Receber")
  IF v_is_natanael
     AND OLD.status = 'em_andamento'::public.compra_status
     AND NEW.status = 'finalizado'::public.compra_status THEN
    RETURN NEW;
  END IF;

  -- Retornos permitidos para responsáveis (origem ou destino)
  IF (
       (OLD.status = 'analise'::public.compra_status AND NEW.status = 'solicitacao'::public.compra_status)
       OR (OLD.status = 'a_receber'::public.compra_status AND NEW.status = 'em_andamento'::public.compra_status)
     )
     AND (
       (v_resp_origem IS NOT NULL AND auth.uid() = v_resp_origem)
       OR (v_resp_destino IS NOT NULL AND auth.uid() = v_resp_destino)
     ) THEN
    RETURN NEW;
  END IF;
