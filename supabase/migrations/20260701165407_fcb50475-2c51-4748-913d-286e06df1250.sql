...
BEGIN
  ...
  -- Em Pendente Aprovação, somente o responsável desse status pode aprovar ou reprovar.
  ...
  -- Fora da aprovação, a movimentação não pode pular etapas: só o próximo status da sequência.
  ...
  -- O responsável do status atual pode empurrar para o próximo, e o responsável do próximo pode puxar.
  IF (v_resp_origem IS NOT NULL AND auth.uid() = v_resp_origem)
     OR (v_resp_destino IS NOT NULL AND auth.uid() = v_resp_destino) THEN
    RETURN NEW;
  END IF;

  -- Compatibilidade para status sem responsável configurado.
